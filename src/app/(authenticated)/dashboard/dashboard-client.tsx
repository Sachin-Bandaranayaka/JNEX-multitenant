...
function StatCard({ title, value, delay }: { title: string; value: string; delay: number }) {
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay }} className="rounded-lg bg-card p-3 sm:p-4 ring-1 ring-border">
            <div className="text-xs sm:text-sm font-medium text-muted-foreground truncate">{title}</div>
            <div className="mt-1 text-lg sm:text-2xl font-semibold text-card-foreground break-words">{value}</div>
        </motion.div>
    );
}
